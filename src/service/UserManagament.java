...
public class UserManagament {
    ...
    public void createNewUser(User newFullName, Rol roles, User currentUser) {
        if (roles.equals(Rol.ADMINISTRATOR)) { // Me crea una condición si el usuario es Administrativo puede utilizar este metodo
            if (countUsers < users.length) { //Si el contador de usuarios es menor a los usuariosque se pueden crear, me deja crear al nuevo usuario
                users[countUsers] = newFullName;
                countUsers++;
                System.out.println("Usuario creado correctamente");

                if (historyCount < actionHistories.length){//si el contador del historial es menor al total de acciones que guarda el arreglo, me deja crear una nueva accion de historial
                    String description = "El administrador " + currentUser.getFullName() +
                            " ha creado un nuevo usuario: " + newFullName.getFullName();
                    ActionHistory newLog = new ActionHistory(description, currentUser); // aqui se envia la descripcion y el usuario que realiza la acción
                    actionHistories[historyCount] = newLog;
                    historyCount++;
                } else {
                    System.out.println("El historial de acciones está lleno."); // muestra un mensaje
                }
            } else {
        ...
        if (roles.equals(Rol.ADMINISTRATOR) || roles.equals(Rol.STANDARD)) { //Con esta condición indicio que tanto el administrador como el Estandar pueden utilizar este metodo
            ...
            for (int i = 0; i < users.length; i++) { //recorro cada usuario hasta encontrar al usuario
                if (users[i] != null && findName.equalsIgnoreCase(users[i].getUsername())) {
                    ...
                    break; // y dejo de buscar, detengo el ciclo
                }
            }
    ...
    public void updateUser(Rol currentUserRol, User currentUser) {
        ...
        if (currentUser != null && historyCount < actionHistories.length && currentUserRol.equals(Rol.ADMINISTRATOR)){
            description3 = "El administrador " + currentUser.getFullName() + "ha actualizado al usuario" + newName;
            ActionHistory newLog = new ActionHistory(description3, currentUser);
    ...
    public void deleteUser(Rol rolUser, User currentUser) {
        ...
        if (rolUser.equals(Rol.ADMINISTRATOR)) {
            ...
            if (currentUser != null && historyCount < actionHistories.length && rolUser.equals(Rol.ADMINISTRATOR)){
            description4 = "El administrador " + currentUser.getFullName() + "ha eliminado al usuario" + deleteName;
            ActionHistory newLog = new ActionHistory(description4, currentUser);
 ...
 */
    public void showHistory(User user){
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss"); // formateo para que el mensaje me lo muestre de esa manera
        boolean found = false;
        for (int i = 0; i < historyCount; i++){ // recorro cada hisotorial de acciones
            ActionHistory ah = actionHistories[i];
            if (ah != null && ah.getPerformigUser().getId().equals(user.getId())){
                String formattedDate = sdf.format(new Date(ah.getTime())); // traigo la fecha y el tiempo
                System.out.printf("%s - %s\n", formattedDate, ah.getDescription()); 
                found = true;
            }
        }if (!found){
            System.out.println("No hay acciones para mostrar"); // si no hay acciones por mostrar, se envia ese mensaje
        }
